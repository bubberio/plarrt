...
subroutine ode ( f, neqn, y, t, tout, relerr, abserr, iflag, work, iwork )
    ...
    !
    !    Input/output, real ( kind = 16 ) Y(NEQN), the current solution.
    !
    !    Input/output, real ( kind = 16 ) T, the current value of the independent
    !    variable.
    !
    !    Input, real ( kind = 16 ) TOUT, the desired value of T on output.
    !
    !    Input, real ( kind = 16 ) RELERR, ABSERR, the relative and absolute error
    ...
    !
    !    Input/output, real ( kind = 16 ) WORK(100+21*NEQN), workspace.
    !
  ...
  told, delsgn, ns, nornd, k, kold, isnold )
    ...
    !
    !    Workspace, real ( kind = 16 ) YY(NEQN), used to hold old solution data.
    !
    !    Input, real ( kind = 16 ) WT(NEQN), the error weight vector.
    !
    !    Workspace, real ( kind = 16 ) P(NEQN).
    !
    !    Workspace, real ( kind = 16 ) YP(NEQN), used to hold values of the
    !    solution derivative.
    !
    !    Workspace, real ( kind = 16 ) YPOUT(NEQN), used to hold values of the
    !    solution derivative.
    !
    !    Workspace, real ( kind = 16 ) PHI(NEQN,16), contains divided difference
    !    information about the polynomial interpolant to the solution.
    !
    !    Workspace, real ( kind = 16 ) ALPHA(12), BETA(12), SIG(13).
    !
    !    Workspace, real ( kind = 16 ) V(12), W(12), G(13).
    !
    ...
    !
    !    Workspace, real ( kind = 16 ) PSI(12), contains information about
    !    the polynomial interpolant to the solution.
    !
    !    Input/output, real ( kind = 16 ) X, a "working copy" of T, the current value
    !    of the independent variable, which is adjusted as the code attempts
    !    to take a step.
    !
    !    Input/output, real ( kind = 16 ) H, the current stepsize.
    !
    !    Input/output, real ( kind = 16 ) HOLD, the last successful stepsize.
    !
    !    Input/output, logical START, is TRUE on input for the first step.
    !    The program initializes data, and sets START to FALSE.
    !
    !    Input/output, real ( kind = 16 ) TOLD, the previous value of T.
    !
    !    Input/output, real ( kind = 16 ) DELSGN, the sign (+1 or -1) of
    !    TOUT - T.
    !
    ...
    real ( kind = 16 ) abseps
    real ( kind = 16 ) abserr
    real ( kind = 16 ) alpha(12)
    real ( kind = 16 ) beta(12)
    logical crash
    real ( kind = 16 ) del
    real ( kind = 16 ) delsgn
    real ( kind = 16 ) eps
    ...
    real ( kind = 16 ) v(12)
    real ( kind = 16 ) w(12)
    real ( kind = 16 ) wt(neqn)
    real ( kind = 16 ) x
    real ( kind = 16 ) y(neqn)
    real ( kind = 16 ) yp(neqn)
    real ( kind = 16 ) ypout(neqn)
    real ( kind = 16 ) yy(neqn)
    !
    ...
    if ( iflag == 1 .or. isnold < 0 .or. delsgn * del <= 0.0D+00 ) then
      ...
      x = t
      yy(1:neqn) = y(1:neqn)
      delsgn = sign ( 1.0D+00, real(del,8) )
  ...
  phi, p, yp, psi, alpha, beta, sig, v, w, g, phase1, ns, nornd )
    ...
    !  Parameters:
    !
    !    Input/output, real ( kind = 16 ) X, the value of the independent variable.
    !
    !    Input/output, real ( kind = 16 ) Y(NEQN), the approximate solution at the
    ...
    !    Input, integer ( kind = 4 ) NEQN, the number of equations.
    !
    !    Input/output, real ( kind = 16 ) H, the suggested stepsize.
    !
    !    Input/output, real ( kind = 16 ) EPS, the local error tolerance.
    !
    !    Input, real ( kind = 16 ) WT(NEQN), the vector of error weights.
    !
    !    Input/output, logical START, is set to TRUE before the first step.
    !    The program initializes data, and resets START to FALSE.
    !
    !    Input/output, real ( kind = 16 ) HOLD, the step size used on the last
    !    successful step.
    !
    ...
    real ( kind = 16 ) fouru
    real ( kind = 16 ) g(13)
    real ( kind = 16 ), dimension ( 13 ) :: gstr = (/ &
  ...
  subroutine intrp ( x, y, xout, yout, ypout, neqn, kold, phi, psi )
    ...
    !  Parameters:
    !
    !    Input, real ( kind = 16 ) X, the point where the solution has been computed.
    !
    !    Input, real ( kind = 16 ) Y(NEQN), the computed solution at X.
    !
    !    Input, real ( kind = 16 ) XOUT, the point at which the solution is desired.
    !
    !    Output, real ( kind = 16 ) YOUT(NEQN), the solution at XOUT.
    !
    !    Output, real ( kind = 16 ) YPOUT(NEQN), the derivative of the solution
    !    at XOUT.
    !
    !    Input, integer ( kind = 4 ) NEQN, the number of equations.
    !
    ...
    !    successful step.
    !
    !    Input, real ( kind = 16 ) PHI(NEQN,16), contains information about the
    !    interpolating polynomial.
    !
    !    Input, real ( kind = 16 ) PSI(12), contains information about the
    !    interpolating polynomial.
    !
    implicit none
...
end module ode_shampine_gordon
